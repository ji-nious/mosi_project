...
public class ProductController {
  ...
  public String uploadSubmit(@ModelAttribute ProductUploadForm form, HttpSession session, Model model) throws IOException {
    ...
    if (form.getNormalPrice() == null || form.getNormalPrice() < 0) {
      model.addAttribute("errorMessage", "가격은 0원 이상이어야 합니다.");
      return "product/product_enroll";
    }
  ...
  public String editSubmit(@PathVariable("id") Long id,
    ...
    // 상품명 검증
    if (form.getProduct().getTitle() == null || form.getProduct().getTitle().trim().isEmpty()) {
      model.addAttribute("errorMessage", "상품명을 입력해주세요.");
      return "product/product_update";
    }
    // 가격 검증
    if (form.getProduct().getNormalPrice() == null || form.getProduct().getNormalPrice() < 0) {
      model.addAttribute("errorMessage", "가격은 0원 이상이어야 합니다.");
      return "product/product_update";
    }
